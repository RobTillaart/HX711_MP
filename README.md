...
HX711_MPC is an **experimental** derived class of HX711 with (5) multipoint calibration.


## Description

Since version 0.4.0 the library implements an **experimental** HX711_MPC derived class.
The current / first MPC version supports 5 points which are mandatory to calibrate.

The added value of the multipoint calibration is to be more exact over the whole range 
of measurements of the load cell. Note that not all load cells are created equally and
that these are seldom linear over the whole working range.

HX711_MPC uses a different calibration scheme. 
...
Same as HX711 with the following extras

- **void getCalibration(uint8_t index, uint32_t \* raw, uint16_t \* weight)** read from internal calibration array
- **void setCalibration(uint8_t index, uint32_t raw, uint16_t weight)** write to internal calibration array
